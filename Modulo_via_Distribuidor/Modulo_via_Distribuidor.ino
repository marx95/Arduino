...
#define Pin_RX                           11 // '- Okay
#define Pin_TX                           10 // '- Okay

//###################################################################################################################################
//###################################################################################################################################
//###################################################################################################################################

#define Pin_TPS                          A14       // Pino entrada tps - é ANALOGICO
#define Pin_Sensor_Temp_Agua             A14       // Sensor Temperatura do Cabeçote
...
#define Pin_Voltimetro_A                 A1       // '- Okay
#define Pin_Voltimetro_B                 A1       // '- Okay
#define Pin_Fonte_A                      22       // '- Okay
#define Pin_Fonte_B                      23       // '- Okay

#define Pin_Sinal_Dist                   48       // '- Okay  Pino Sinal do Distribuidor, Calcula RPM com isto
#define Pin_Rele_Bob                     32       // '- Okay
#define Pin_Rele_BombaCombus             33       // '- Okay

#define Pin_Recuo_Atuador_ML             30      // '- Okay

#define Pin_Ci_Bico_1                    40     // '- Okay
#define Pin_Ci_Bico_2                    41     // '- Okay
#define Pin_Ci_Bico_3                    42     // '- Okay
#define Pin_Ci_Bico_4                    43     // '- Okay
...
float Voltagem_Bateria_A            = 0;
float Voltagem_Bateria_B            = 0;
...
// DEFINIÇAO PACOTES
char Pack_TensaoBat_A[8];
char Pack_TensaoBat_B[8];
  ...
  if(t_Agr - 150 > Tempo_TensaoBat)
  {
    ...
    */
    ...
    char str_temp[6];
      ...
      T_Total       = (analogRead(Pin_Voltimetro_A)/2) * 4;
      DezPCent      = (T_Total/100) * 22;
      T_Total        = T_Total + DezPCent;
      Voltagem_Bateria_A  = T_Total / 100;
      dtostrf(Voltagem_Bateria_A, 4, 1, str_temp);
      //sprintf(Pack_TensaoBat_A, "%s", str_temp);

      T_Total       = (analogRead(Pin_Voltimetro_B)/2) * 4;
      DezPCent      = (T_Total/100) * 22;
      T_Total       = T_Total + DezPCent;
      Voltagem_Bateria_B  = T_Total / 100;
      dtostrf(Voltagem_Bateria_B, 4, 1, str_temp);
      //sprintf(Pack_TensaoBat_A, "%s", str_temp);
      
    }
    
    
    sprintf(Msg, "Bat: %s x %s", Voltagem_Bateria_A, Voltagem_Bateria_B);
    Log(Msg);
...
void loop()
{
 ...
 // Verifica_DeadTime();
  ...
  {
      int bt_Recv = bluetooth.read();

      if(bt_Recv == 65) InfStatusUpdate();
      sprintf(Msg, "Request Update(%d)", bt_Recv);
      //bluetooth.println("RPM:");
     Log(Msg);
  }
  //####################################################################################################
  return;
  //sprintf(Msg, "%d", digitalRead(48));
  //Log(Msg);
  //return;
  ...
  sprintf(Msg, "[dst: %d]", digitalRead(Pin_Sinal_Dist));
  //Log(Msg);
  ...
  if(bluetooth.available())
  {
      ...
      if(bt_Recv == 65) InfStatusUpdate();
      sprintf(Msg, "Recebido: %d", bt_Recv);
      //bluetooth.println("RPM:");
     Log(Msg);
  }
  //####################################################################################################
...
void InfStatusUpdate()
{
    sprintf(Msg, "#InfUp:%s;%s;$", Pack_TensaoBat_A, Pack_TensaoBat_B);
    Log(Msg);
    ...
    char floatStr[5];
    char floatStrB[5];
    //dtostrf(Voltagem_Bateria, 4, 1, floatStr);

    float Voltagem_Bateria2;
    //Voltagem_Bateria2 = Voltagem_Bateria;
    dtostrf(Voltagem_Bateria2, 4, 1, floatStrB);

    
    String AtuadorML_PKG ;
    AtuadorML_PKG = "0";
    if(CutOffStatus == true) AtuadorML_PKG = "1";
    AtuadorML_PKG = "0";
 
    sprintf(Msg, "#InfUp:%s;%s;%d;%d;%d;%d;%d;%s;$", floatStr, floatStrB, TPS, Bicos_DeadTime, Bobina_DeadTime, Tempo_Injecao, RPM, AtuadorML_PKG);
    bluetooth.println(Msg);
    
    //sprintf(Msg, "TPS: %d%%", TPS);
    //bluetooth.println(Msg);
   ...
   // bluetooth.println(Msg);
    ...
    //sprintf(Msg, "Tempo Inj: %d%s [%d]", tmpDl, "ms", Tempo_Injecao);
    //bluetooth.println(Msg);
    //bluetooth.println("$");
}
